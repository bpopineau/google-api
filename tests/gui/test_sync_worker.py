...
def test_sync_worker_success(qtbot, mock_clients, tmp_path):
    ...
    mock_clients.sheets.batch_write.return_value = {
        "updatedRows": 3
    }  # Headers + 2 files

    with qtbot.waitSignals(
        [worker.started_scan, worker.started_upload, worker.finished], timeout=5000
    ):
        worker.start()
...
def test_sync_worker_error(qtbot, mock_clients, tmp_path):
    ...
    with patch(
        "mygooglib.core.utils.file_scanner.FileScanner.scan",
