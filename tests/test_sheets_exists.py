...
def test_exists_success(client, mock_service):
    # Setup
    spreadsheet_id = "some_valid_id_longer_than_20_chars_xyz"
...
def test_exists_not_found(client, mock_service):
    # Setup
    spreadsheet_id = "missing_id_longer_than_20_chars_xyz_123"
    resp = MagicMock()
    resp.status = 404
...
def test_exists_with_title_resolution(client, mock_service, mock_drive):
    ...
    with patch(
        "mygooglib.services.sheets.resolve_spreadsheet", return_value=spreadsheet_id
    ):
        mock_service.spreadsheets.return_value.get.return_value.execute.return_value = {
            "spreadsheetId": spreadsheet_id
        }
...
def test_exists_resolution_fails(client, mock_service):
    ...
    with patch(
        "mygooglib.services.sheets.resolve_spreadsheet",
