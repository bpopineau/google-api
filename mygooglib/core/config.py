...
class AppConfig:
    ...
    def config_dir(self) -> Path:
        """Return the directory where config is stored, with migration logic."""
        new_path = Path.home() / ".mygoog"
        old_path = Path.home() / ".mygooglib"

        # Migration from ~/.mygooglib to ~/.mygoog
        if old_path.exists() and not new_path.exists():
            try:
                old_path.rename(new_path)
                logger.info(f"Migrated config from {old_path} to {new_path}")
            except OSError as e:
                logger.error(f"Failed to migrate config directory: {e}")
                return old_path
