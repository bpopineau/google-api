...
def refresh_cmd(ctx: typer.Context) -> None:
    ...
    if creds is None:
        state.console.print("[red]No token found. Run 'mg auth login' first.[/red]")
        raise typer.Exit(code=1)

    if not creds.refresh_token:
        state.console.print(
            "[red]Token has no refresh_token. Re-run 'mg auth login'.[/red]"
        )
        raise typer.Exit(code=1)
    ...
    try:
        ...
        state.console.print(
            "[yellow]Your token may be revoked. Delete token.json and run 'mg auth login'.[/yellow]"
        )
        raise typer.Exit(code=1)
